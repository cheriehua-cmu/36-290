...
This lab is to be begun in class, but may be finished outside of class at any time prior to Wednesday, September 9<sup>th</sup> at 6:00 PM. You must commit both the edited `Rmd` file and the "knitted" `html` file for this lab to your `GitHub` repo. (You can verbally discuss aspects of the labs with your classmates, but please do not share code, etc.)
...
```
```
Length() returns 6 because there are 6 columns in predictors.
```
...
```
```
u, g, z, and y have max values of 99.
```

## dplyr

Below, we will practice using `dplyr` package functions to select rows and/or columns of a data frame. `dplyr` is part of the `tidyverse`, and is rapidly becoming the most oft-used way of transforming data. To learn more about "transformatory" functions, read through today's class notes, then through Chapter 3 (book; Chapter 5 on-line) of *R for Data Science* (see the syllabus for the link, or just Google for it). In short, you can
...
```{r}
head(arrange(filter(predictors, i < 25 & g > 22)))
```
...
```
```
There are 11 values that are 99.
```
...
```{r}
predictors %>% filter(u == 99 | g == 99 | r == 99 | i == 99 | z == 99 | y == 99)
```
...
```{r}
pred.new <- predictors %>% filter(u != 99 & g != 99 & r != 99 & i != 99 & z != 99 & y != 99)
```
...
df.new <- df %>% filter(u != 99 & g != 99 & r != 99 & i != 99 & z != 99 & y != 99)
pred.new <- df[1:6]
resp.new <- df[, 7]
```
...
```{r}
pred.new <- pred.new %>% mutate(gr = g - r, ri = r + i)
```
...
pred.new %>% 
  group_by(type) %>%
  summarize(gr_med = median(gr), ri_med = median(ri), gr_mean = mean(gr), ri_mean = mean(ri))
```
...
pred.new %>% 
  group_by(type) %>%
  summarize(gr_sd = sd(gr) / sqrt(4000), ri_sd = sd(ri) / sqrt(4000))
```
