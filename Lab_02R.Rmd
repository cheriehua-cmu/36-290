...
This lab is to be begun in class, but may be finished outside of class at any time prior to Friday, September 11<sup>th</sup> at 6:00 PM. You must commit both the edited `Rmd` file and the "knitted" `html` file for this lab to your `GitHub` repo. (You can verbally discuss aspects of the labs with your classmates, but please do not share code, etc.)
...
Note that `ggplot` expects data frames (or tibbles...which we may use in future labs), so I had to add the call to `data.frame`. Also note the structure here: the first function is `ggplot()`, and you pass a data frame (as `data`) and an identification of which feature goes along which axis (as `mapping`). Once you've identified the data and the mapping, you "add on" another function that describes what you will do to the data; in this case, that function is `geom_histogram()`.
...
```{r}
ggplot(data=data.frame(response),mapping=aes(x=response)) + geom_histogram(fill = 'olivedrab1', bins = 60)
```
```
The data seems to be skewed to the left. The mode is around 0.4.
```
...
```{r}
ggplot(data=data.frame(response),mapping=aes(x=response)) + geom_histogram(fill = 'olivedrab1', bins = 60) + labs(x = "Redshift z", y = "Number of Galaxies at Redshift z")
```
...
```{r}
ggplot(data=data.frame(predictors),mapping=aes(x=type)) + geom_bar(fill = 'olivedrab1')
```
...
```{r}
ggplot(data=data.frame(predictors),mapping=aes(x=u)) + geom_histogram(fill = 'olivedrab1', bins = 60) + xlim(20, 32) + facet_wrap(~ type)
```
...
```{r}
ggplot(data=data.frame(predictors),mapping=aes(x=u,fill=type)) + 
...
```{r}
ggplot(data=data.frame(predictors),mapping=aes(x=type,y=u)) + geom_boxplot(fill = 'olivedrab1') 
```
...
```{r}
head(gather(pred.notype))
```
```
The names of the columns are "key" and "value."
```
...
```{r}
ggplot(data=gather(pred.notype),mapping=aes(x=value)) + geom_histogram(fill="olivedrab1") +
  facet_wrap(~key) + xlim(20, 32) + labs(x = "Magnitude")
```
...
```{r}
ggplot(data=data.frame(predictors), mapping = aes(x=r, y=response)) + geom_point(color = "olivedrab1", size=0.5) + labs(x = "r-band magnitude", y = "Redshift")
```
...
```{r}
ggplot(data=data.frame(predictors), mapping = aes(x=r, y=response)) + geom_point(color = "olivedrab1", size=0.5) + labs(x = "r-band magnitude", y = "Redshift") + geom_density_2d()
```

## Question 11

Repeat Q10, but use `geom_hex()` instead.
```{r}
ggplot(data=data.frame(predictors), mapping = aes(x=r, y=response)) + geom_point(color = "olivedrab1", size=0.5) + labs(x = "r-band magnitude", y = "Redshift") + geom_hex()
```
