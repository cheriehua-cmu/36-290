...
date: "Week 1 Thursday -- Fall 2020"
output: 
  html_document:
    toc: true
    toc_float: true
...
This lab is to be begun in class, but may be finished outside of class at any time prior to Friday, September 4<sup>th</sup> at 6:00 PM. You must commit both the edited `Rmd` file and the "knitted" `html` file for this lab to your `GitHub` repo. (You can verbally discuss aspects of the labs with your classmates, but please do not share code, etc.) See below for more information on what precisely "a knitted HTML file" is.
...
In every question, you will see a code chunk. A code chunk begins witha line with "\`\`\`{r}" (three backquotes, a left curly bracket, an r, and a right curly bracket, and without the double quote marks shown here), (a) line(s) containing code, and a final line with "\`\`\`". Here is an example (that only truly makes sense if you are looking at the `Rmd` file itself):
```{r}
print("Hello, world.")
```
...
```
```
Yeah, it's the number of values that have x == 1.
```
